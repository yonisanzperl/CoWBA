...
function [Answer,Answer0,Cancelled]=IN_CoWBA_mb1(varargin)
% % 
% if nargin==1
%     Answer0=varargin{1};
% 
...
Formats(3,3).items = {'there is not .mat from model-free'};
Formats(3,3).limits = [1 1]; % multi-select
Formats(3,3).size =  size_control;
DefAns.allIt = {'there is not .mat from model-free'};
...
Prompt(end+1,:) = {'Which Model?','nModel',[]};
Formats(4,4).labelloc = 'topcenter';
Formats(4,4).type = 'list';
Formats(4,4).style = 'list';
Formats(4,4).format = 'text'; % Answer will give value shown in items, disable to get integer
Formats(4,4).items = {'Non-linear Hopf','Linear Hopf','Ising'};
Formats(4,4).limits = [1 1]; % multi-select
Formats(4,4).size =  size_control;
DefAns.nModel = {'Non-linear Hopf'};
%Formats(7,2).callback = @(~,~,h,k)disableG(h,k);
...
Formats(7,2).callback = @(~,~,h,k)disablePerts(h,k);


Prompt(end+1,:) = {'Which Observable do you Fit?','nFit',[]};
Formats(7,4).labelloc = 'topcenter';
Formats(7,4).type = 'list';
Formats(7,4).style = 'list';
Formats(7,4).format = 'text'; % Answer will give value shown in items, disable to get integer
Formats(7,4).items = {'FC','FCD','Metastability','Synchronization','Integration'};
Formats(7,4).limits = [1 1]; % multi-select
Formats(7,4).size =  size_control;
DefAns.nFit = {'FC'};
%Formats(7,2).callback = @(~,~,h,k)disableG(h,k);
