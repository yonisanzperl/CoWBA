function CoWBA_plot_empirical_nice(output,Cfg,metadata)
...
% 
% K=3; % arbitrary number to create a meaningful transfer
% NoComp = Cfg.nBrainStates*(Cfg.nBrainStates-1)*0.5;
...
for ii=1:Cfg.nBrainStates
    metaWrapper{1,ii}= output(ii).meta;
    syncWrapper{1,ii}= output(ii).sync;
    C_normWrapper{1,ii}= mean(squeeze(output(ii).C_norm),1);
    meanFCDWrapper{1,ii}= mean(squeeze(output(ii).FCD_iFC_triu),1);
    clear tmp
end

% Metastability
figure('Name','Metastability');
p = swarm(metaWrapper, groups, tlt='', overlay_style='boxplot', printPvals=true, stat_test=stattest,name='ModelfreemetaStats.txt');
movefile('ModelfreemetaStats.txt', metadata.outdir)

% Synchrony
figure('Name','Synchrony');
p = swarm(syncWrapper, groups, tlt='', overlay_style='boxplot', printPvals=true, stat_test=stattest,name='ModelfreesyncStats.txt');
movefile('ModelfreesyncStats.txt', metadata.outdir)

% LZ Complexity
figure('Name','LZ Complexity');
p = swarm(C_normWrapper, groups, tlt='', overlay_style='boxplot', printPvals=true, stat_test=stattest,name='ModelfreeLCzStats.txt');
movefile('ModelfreeLCzStats.txt', metadata.outdir)

% mean FCD
figure('Name','mean FCD');
p = swarm(meanFCDWrapper, groups, tlt='', overlay_style='boxplot', printPvals=true, stat_test=stattest,name='ModelfreemeanFCDStats.txt');
movefile('ModelfreemeanFCDStats.txt', metadata.outdir)
